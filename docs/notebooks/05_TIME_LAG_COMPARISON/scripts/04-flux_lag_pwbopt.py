...
# When launched by run_parallel.py set TLAG_NO_DISPLAY=1 to suppress windows.
if _os.environ.get('TLAG_NO_DISPLAY'):
    import matplotlib as _mpl

    _mpl.use('Agg')
_NO_DISPLAY = bool(_os.environ.get('TLAG_NO_DISPLAY'))
...
INPUT_DIR = Path(
    r"F:\Sync\luhk_work\20 - CODING\29 - WORKBENCH\ms_fluxnet_ch4_n2o_timelag\input\03-rotated_data_from_eddypro_level5_part1")
...
OUTPUT_DIR = Path(
    r"F:\Sync\luhk_work\20 - CODING\29 - WORKBENCH\dataset_ch-cha_flux_product\docs\notebooks\05_TIME_LAG_COMPARISON\output")

# Allow INPUT_DIR / OUTPUT_DIR to be overridden from the command line
# when this script is launched by run_parallel.py.
import argparse as _ap

_p = _ap.ArgumentParser(add_help=False)
_p.add_argument('--input-dir', type=Path)
_p.add_argument('--output-dir', type=Path)
_p.add_argument('--file-list', type=Path,
                help='text file with one input file path per line; overrides --input-dir')
_cli, _ = _p.parse_known_args()
if _cli.input_dir is not None:
    INPUT_DIR = _cli.input_dir
...
if not _csv_loaded and USE_SYNTHETIC:
    ...
    # ---------- real-data mode ----------
    if _cli.file_list is not None:
        if not _cli.file_list.exists():
            raise FileNotFoundError(f'--file-list not found: {_cli.file_list}')
        file_list = [Path(p.strip()) for p in _cli.file_list.read_text().splitlines()
                     if p.strip()]
        print(f'Real-data mode: {len(file_list)} files from file list {_cli.file_list}')
    else:
