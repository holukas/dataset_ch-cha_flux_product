...
After the creation of the individual quality flags, they are combined into one overall quality control flag `QCF`. However, the `QCF` at this step is not the final overall quality flag. It is only needed to *temporarily* remove already rejected values from the dataset before outlier detection in Level-3.2. This means the outlier detection algorithms run on *temporarily* filtered data. 
