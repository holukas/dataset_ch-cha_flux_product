...
- Single flags from Level-2 are combined into one quality control flag `QCF`.
- The `QCF` from this step is later *temporarily* applied to Level-3.1 (storage-corrected) fluxes for outlier detection tests in Level-3.2.
- For building the overall flag after Level-3.3, the individual test results from Level-2 are used.
